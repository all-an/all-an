...
int liberamapa(MAPA* m) {    //ponteiro apontando para m, pode ser passado qualquer mapa futuramente criado
    for(int i = 0; i < m->linhas; i++){ //m->linhas È o mesmo que ((*m).linhas) onde *m È o conte˙do do ponteiro
        free(m->matriz[i]);
    }
    free(m->matriz);
}
...
int lemapa(MAPA* m) {
    ...
    fscanf(f, "%d  %d", &(m->linhas), &(m->colunas));//ler do arquivo txt e armazenar nas vari√°veias
